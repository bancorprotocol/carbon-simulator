...
class CPCArbOptimizer(OptimizerBase):
    ...
    class NofeesOptimizerResult(ConvexOptimizerResult):
        ...
        def __post_init__(self, optimizer=None, problem=None, dx=None, dy=None, *args, **kwargs):
        ...
        def trade_instructions(self, ti_format=None):
            ...
            """
            result = (
                CPCArbOptimizer.TradeInstruction.new(curve_or_cid=c, tkn1=c.tknx, amt1=dx, tkn2=c.tkny, amt2=dy)
                for c, dx, dy in zip(self.curves, self.dxvalues, self.dyvalues) if dx!=0 or dy!=0
            )
    ...
    class SimpleOptimizerResult(OptimizerBase.OptimizerResult):
        ...
        *the CPCInverter object ensures that all curves in the list correspond to the same quote
        ...
        def trade_instructions(self, ti_format=None):
            """returns list of TradeInstruction objects"""
            result = (
                CPCArbOptimizer.TradeInstruction.new(curve_or_cid=c, tkn1=self.tknx, amt1=dx, tkn2=self.tkny, amt2=dy)
                for c, dx, dy in zip(self.curves, self.dxvalues, self.dyvalues) if dx!=0 or dy!=0
            )
            return CPCArbOptimizer.TradeInstruction.to_format(result, ti_format)
    ...
    def margp_optimizer(self, sfc=None, result=None, *, params=None):
        ...
        **at the moment only the trivial self-financing constraint is allowed, ie the one that
        ...
        assert targettkn in alltokens_s, f"targettkn {targettkn} not in {alltokens_s}"
        tokens_t = tuple(t for t in alltokens_s if t!=targettkn)
        tokens_ix = {t: i for i, t in enumerate(tokens_t)}
        if not P("pstart") is None:
            ...
            try:
                ...
                if P("verbose") or P("debug"):
                    print("[margp_optimizer] error while calculating price estimates:", e)
        ...
        get = lambda p, ix: p[ix] if ix is not None else 1
        
        dxdy_f = lambda r: (np.array(r[0:2]))
        curves_by_pair = {
            pair: tuple(c for c in curves_t if c.pair == pair) 
            for pair in pairs
        }
        ...
        for i in range(maxiter):
            ...
            #J = self.J(dtknfromp_f, p)
            J = self.J(dtknfromp_f, plog10) # ATTENTION: dtknfromp_f takes log10(p) as input
            if P("debug"):
            ...
            if P("verbose"):
                ...
                print(f"[criterium={criterium:.2e}, eps={eps:.1e}, c/e={criterium/eps:,.0e}]")
                if P("tknd"):
                    print("dtkn_d", dtkn_d)
                if P("J"):
                    print("J", J)
        ...
        if i == maxiter-1:
            return self.MargpOptimizerResult(
                ...
                tokens_t = tokens_t,
                n_iterations = i,
                errormsg = f"maximum number of iterations reached [{maxiter}]",
            )
    ...
    class MargpOptimizerResult(OptimizerBase.OptimizerResult):
        ...
        def __post_init__(self, *args, **kwargs):
            ...
            assert self.p_optimal_t is not None or self.errormsg is not None, "p_optimal_t must be set unless errormsg is set"
            if self.method is None:
                self.method = "margp"
        ...
        def dxdyvalues(self, asdict=False):
            ...
            assert self.is_error is False, "cannot get this data from an error result"
            result = ((c.cid, c.dxdyfromp_f(self.price(c.tknb, c.tknq))[0:2]) for c in self.curves)
            if asdict:
        ...
        def curves_new(self):
            """returns a list of Curve objects the trade instructions implemented"""
            assert self.optimizer is not None, "optimizer must be set [do not use minimal results]"
            assert self.is_error is False, "cannot get this data from an error result"
            return self.optimizer.adjust_curves(dxvals = self.dxvalues)
    ...
    class TradeInstruction(_DCBase):
        ...
        def __post_init__(self, curve=None):
            ...
            if curve is not None:
                assert self.cid == curve.cid, f"curve/cid mismatch [{self.cid} vs {curve.cid}]"
            ...
            if not self.amtin*self.amtout < 0:
                if abs(self.amtin)<self.POSNEGEPS and abs(self.amtout)<self.POSNEGEPS:
                    ...
                    raise ValueError(f"amtin and amtout must be of different sign [{self.amtin} {self.tknin}, {self.amtout} {self.tknout}]")
            assert self.amtin >= 0, f"amtin must be positive [{self.amtin}]" # seen from AMM
        ...
        def new(cls, curve_or_cid, tkn1, amt1, tkn2, amt2, *, eps=None):
            ...
            if amt1 > 0:
                newobj = cls(cid=cid, tknin=tkn1, amtin=amt1, tknout=tkn2, amtout=amt2, curve=curve)
            else:
                newobj = cls(cid=cid, tknin=tkn2, amtin=amt2, tknout=tkn1, amtout=amt1, curve=curve)
