...
class OrderBookSimple():
    ...
    - Price chart: plots the effective (1) and marginal (2) prices vs out amount
        :x1:    ob.out_amounts
        :y1:    ob.prices
        :x2:    ob.midpoints(ob.out_amounts)
        :y2:    ob.marg_prices
    ...
    def prices(self):
        """
        returns the effective prices (in_amounts / out_amounts)
        """
        return self.in_amounts / self.out_amounts
    ...
    def marg_prices(self):
        """
        returns the reverse marginal prices (marg_out_amounts / marg_in_amounts)
        """
        return self.marg_in_amounts / self.marg_out_amounts 
    ...
    def prices_u(self):
        ...
        unit of `prices`, `marg_prices`, etc
        """
        return f"{self.in_tkn} per {self.out_tkn}"
    marg_prices_u = prices_u
    ...
    def pricesr_u(self, reverse=True):
        ...
        unit of `pricesr`, `marg_pricesr`, etc
        """
        return f"{self.out_tkn} per {self.in_tkn}" if reverse else self.prices_u
    marg_pricesr_u = pricesr_u
    ...
    def ob_liquidity(self):
        ...
        returns the order book liquidity at the price points corresponding to `ob_prices`
        """
        return self.midpoints(self.marg_in_amounts) / self.dmarg_prices
    ...
    def __repr__(self):
        return f"{self.__class__.__name__}(<out={self.out_tkn} in={self.in_tkn}; {len(self.out_amounts)-1} records>)"
