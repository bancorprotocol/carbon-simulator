...
class Node(_DCBase):
    ...
    @dataclass
    class State(_DCBase):
        amount_: TrackedStateFloat = field(default_factory=TrackedStateFloat, init=False)
        amount: InitVar = None
...
class Edge(_DCBase):
    ...
    @classmethod
    def connection_edge(cls, node_in, node_out, price=None, inverse=False, weight=None, ix=None, uid=None):
        """
        ...
        return cls(node_in=node_in, amount_in=-weight, node_out=node_out, amount_out=-price*weight, ix=ix, inverse=inverse, uid=uid)
    ...
    def state(self):
        try:
            ...
            raise ValueError("edge state not initialized (only available on Amount edges))")
    ...
    def __add__(self, other):
        ...
        """
        if other == 0: return self.duplicate() # required for sum() to work
        if not isinstance(other, self.__class__):
            raise ValueError(f"cannot add {self.__class__} and {other.__class__}")
        assert self.edgetype == other.edgetype, "arithmetic operations only allowed on edges of the same type"
        if not (self.node_out is other.node_out and self.node_in is other.node_in):
            raise ValueError(f"nodes do not match", self, other)
        return self.__class__(self.node_in, self.amount_in+other.amount_in, 
                        self.node_out, self.amount_out+other.amount_out, inverse=self.inverse)
    ...
    def __mul__(self, other):
        ...
        return self.__class__(self.node_in, self.amount_in*other, 
                              self.node_out, self.amount_out*other, inverse=self.inverse)
    ...
    def duplicate(self):
        ...
        """
        return self.__class__(self.node_in, self.amount_in, self.node_out, self.amount_out, None, self.inverse)

    def reverse(self):
        """
        duplicates an edge but reverses it
        """
        return self.__class__(self.node_out, self.amount_out, self.node_in, self.amount_in, None, not self.inverse)
    R = reverse
    ...
    def pair(self, inverse=None):
        ...
        return f"{self.tkn_in}/{self.tkn_out}" if not inverse else f"{self.tkn_out}/{self.tkn_in}"
    ...
    def convention(self, inverse=None):
        ...
        if inverse is None:
            inverse = self.inverse
        return f"{self.tkn_in} per {self.tkn_out}" if inverse else f"{self.tkn_out} per {self.tkn_in}"
    ...
    def price(self, inverse=None):
        ...
        if inverse is None:
            inverse = self.inverse
        return self.amount_in / self.amount_out if inverse else self.amount_out / self.amount_in
    p = price
    ...
    def transport(self, amount_in=None, record=False, raiseonerror=True):
        ...
        if isinstance(amount_in, Amount):
            assert amount_in.tkn is self.node_in, f"amount token {amount_in.tkn} does not match input node {self.node_in}"
            amount_in = amount_in.amount

        if self.is_amounttype:
            # those checks only make sense for amounttype edges
            assert amount_in <= self.amount_in+EPS, f"amount {amount_in} exceeds edge capacity {self.amount_in}"
        ...
        if record:
            ...
            if self.state.amount_in_remaining < -EPS:
                if raiseonerror:
                    raise ValueError(f"amount {amount_in} exceeds remaining edge capacity {self.state.amount_in_remaining}")
            self.node_out.state.amount += amount_out
            self.node_in.state.amount -= amount_in
            if self.node_in.state.amount < -EPS:
                if raiseonerror:
                    raise ValueError(f"amount {amount_in} exceeds node capacity {self.node_in.state.amount}")
        return Amount(amount_out, self.node_out)

    def __str__(self):
        arrow = "-->" if self.ix is None else f"--({self.ix})->"
        return f"{self.amount_in} {self.node_in} {arrow} {self.amount_out} {self.node_out}"

    @property
    def label(self):
        if self.is_amounttype:
            return f"{self.amount_in} {self.node_in} --> {self.amount_out} {self.node_out}"
        else:
...
class Cycle(_DCBase):
    ...
    def __str__(self):
        try:
            return f"cycle [{self.uid}]: "+"->".join([ f"{d.tkn}" for d in self.data])+"->..."
        except:
            return f"cycle [{self.uid}]: "+"->".join([ f" {d} " for d in self.data])+"->..."
        ...
        * only one of start_ix and start_val can be specified
        ...
        if not start_val is None:
            if not start_ix is None:
                raise ValueError("only one of start_ix and start_val can be specified", start_ix, start_val)
...
class ArbGraph(_DCBase):
    ...
    @classmethod
    def from_ccdxdy(cls, cc, dxv, dyv, verbose=False):
        """
    ...
    @classmethod
    def from_r(cls, r, verbose=False):
        """
        alternative constructor: from an Optimizer result object
        
        :r:     Optimizer result object
        """
        return cls.from_ccdxdy(r.curves, r.dx.value, r.dy.value, verbose=False)
    ...
    def duplicate(self, consolidate=True):
        ...
        if not consolidate:
            edges = (Edge(edge.node_in, edge.amount_in, edge.node_out, edge.amount_out, 
                                        edge.inverse, uid=edge.uid) for edge in self.edges)
        else:
            edges = ( self.filter_edges(nin, nout)
                            for nin in self.nodes for nout in self.nodes if nin != nout)
    ...
    def state(self):
        ...
        """
        return self.State(nodes=tuple(node.state for node in self.nodes),
    ...
    def add_edge_obj(self, edge):
        ...
        if edge.edgetype != self.edgetype and not self.edgetype is None:
            raise ValueError("edge type does not match graph type", edge.edgetype, self.edgetype)
        edge.set_ix(len(self.edges))
        self.edges.append(edge)
        return self

    def add_edge(self, tkn_in, amount_in, tkn_out, amount_out, inverse=False, uid=None):
        """
    ...
    def add_edge_connectiontype(self, tkn_in, tkn_out, price=None, inverse=False, price_outperin=None, weight=None, symmetric=True, uid=None):
        """
        ...
        **the weight of the connection is mostly useful to determine the prices of combined
        ...
        if price_outperin is not None:
            ...
            assert not inverse, "inverse must be False (=default) if price_outperin is specified"
    ...
    def node_by_tkn(self, tkn, create=False):
        ...
        if isinstance(tkn, Node):
            ...
            assert tkn is node, f"the tkn provided {tkn} does not match the node found {node}"
            return node
        try:
    ...
    def filter_edges(self, node_in=None, node_out=None, node=None, bothways=None, pair=None):
        """
        ...
        """
        if not pair is None:
            assert node_in is None and node_out is None, "cannot specify both pair and node_in or node_out"
            assert node is None, "cannot specify both pair and node"
            node_in, node_out = pair.split("/")
        if bothways is None:
            bothways = False if pair is None else True
        if bothways:
            l1 = self.filter_edges(node_in=node_in, node_out=node_out, node=node, bothways=False)
            l2 = self.filter_edges(node_in=node_out, node_out=node_in, node=node, bothways=False)
            return l1 + l2
        ...
        if node is not None:
            assert node_in is None and node_out is None, "cannot specify both node and node_in or node_out"
        assert node_in is None or isinstance(node_in, Node), f"node_in must be a Node object or None, not {node_in}"
        assert node_out is None or isinstance(node_out, Node), f"node_out must be a Node object or None, not {node_out}"
        assert node is None or isinstance(node, Node), f"node must be a Node object or None, not {node}"
        if not node is None:
            return [edge for edge in self.edges if edge.node_in == node or edge.node_out == node]
            ...
            return [edge for edge in self.edges if edge.node_in == node_in and edge.node_out == node_out]
    fe = filter_edges
    ...
    def as_graph(self, directed=True, weighted=False):
        ...
        for edge in self.edges:
            if weighted:
                ...
                G.add_edge(edge.node_in.ix, edge.node_out.ix, weight="bla", label=str(edge))
            else:
    ...
    def price(self, node_tknb, node_tknq, with_units=False):
        ...
        if node_tknb != node_tknq:
            ...
            price = 1
        if with_units:
            return price, f"{node_tknq.tkn} per {node_tknb.tkn} [{node_tknb.tkn}/{node_tknq.tkn}]"
        return price
    ...
    def cycles(self, asgenerator=False):
        ...
        cycles = (Cycle(cycle, uid) for uid, cycle in enumerate(cycles))
        if asgenerator: return cycles
        return tuple(cycles)
    ...
    class RunArbCycleResult(_DCBase):
        ...
        def asdict(self, include_cycle=True):
            dct =  {**asdict(self), "length": self.length, "r": self.r, 
                    "rpcs": self.rpcs, "ppcs": self.ppcs, "uid": self.cycle.uid}
    ...
    def run_arbitrage_cycle(self, cycle, token=None, **params):
        ...
        current_multiplier = 1.       # tracks how much of the initial amount can be pushed through the cycle
        current_amount_in = None      # tracks the amount currently being pushed (None: to be initialised)
        ...
        if not token is None:
            try:
                ...
                if P("allow_any_token"):
                    ...
                    if P("verbose"):
                        print(f"token {token} not found in cycle {cycle}; first token of cycle used instead")
                else:
                    raise ValueError(f"token {token} does not exist, or not found in cycle {cycle}")
        else:
            cycle_pairs = cycle.pairs()
        ...
        for pair in cycle_pairs:
            ...
            if current_amount_in is None:
                ...
                initial_amount_in = capacity_in    # we remember how much we had at the beginning...
            ...
            current_amounts_out = [ee.transport(amt_in).amount for ee, amt_in in zip(edges, current_amounts_in)]
        ...
        assert initial_tkn_in == e.tkn_out, f"In and out tokens do not match!! {initial_tkn_in}, {e.tkn_out}"
    ...
    def run_arbitrage_cycles(self, cycles, token=None, format=None, **params):
        ...
        arbcycles = (self.run_arbitrage_cycle(cycle, token=token, **params) for cycle in cycles)
    ...
    def run_arbitrage_cyclesf(self, rawresults, format=None):
        ...
        dfa = df1.pivot_table(index="token", values=["profit", "amount_in", "amount_out"])
    ...
    def transport(self, amount_in, tkn_in, tkn_out, record=True, routingalgo=None, raiseonerror=True):
        """
        ...
        if not routingalgo in [self.TPROUT_PRORATA]:
            raise ValueError(f"routingalgo {routingalgo} not supported; see TPROUT_* constants")
        ...
        if isinstance(amount_in, Amount):
            if amount_in.token != node_in.token:
                raise ValueError(f"amount_in token {amount_in.token} does not match node_in token {node_in.token}")
            amount_in = amount_in.amount
        ...
        assert routingalgo == self.TPROUT_PRORATA, f"routingalgo {routingalgo} not supported; use TPROUT_PRORATA"
        routing_factor = amount_in / capacity_in
        amounts_in = [amt_in * routing_factor for amt_in in capacities_in]
        print(f"routing_factor: {routing_factor}; amounts_in: {amounts_in} {amount_in} {capacity_in}")

        # transport the amounts through the edges
        amounts_out = []
        for edge, amt_in in zip(edges, amounts_in):
            amounts_out += [edge.transport(amt_in, record=record, raiseonerror=raiseonerror)]

        return self.TransportResult(
            amount_in = Amount(amount_in, node_in.tkn),
            amount_out = Amount(sum([amt_out.amount for amt_out in amounts_out]), node_out.tkn),
    ...
    def edgedf(self, edges=None, consolidated=True, resetindex=False):
        ...
        if self.is_amounttype:
            ...
            df = pd.concat([dff.reset_index(drop=True), dft[["n"]].rename(columns={"n":"n_rev"}).reset_index(drop=True)], axis=1)
    ...
    def edge_statistics(self, node_in=None, node_out=None, edges=None, pair=None, bothways=False):
        """
        ...
        if bothways:
            return (
                self.edge_statistics(node_in, node_out, edges, bothways=False),
                self.edge_statistics(node_out, node_in, edges, bothways=False),
            )
        if not pair is None:
            assert node_in is None and node_out is None, f"cannot specify both pair and node_in/node_out {pair}, {node_in}, {node_out}"
            node_in, node_out = pair.split("/")
        ...
        if not edges is None:
            assert node_in is None and node_out is None, "cannot specify both edges and node_in/node_out"
        ...
        utilizations = tuple(1-r/a for r,a in zip(amounts_in_remaining, amounts_in))
