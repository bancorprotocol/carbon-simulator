...
# SHIB/USDC
user_y,user_z,user_A,user_B,user_S,xS = encode_user_inputs(1/0.00001453, 1/0.00001555, 100000000, 100000000, 6, 18, ONE_EXPONENT) ## SHIB-side order
...
print(y,z,A,B,S)
print("\n")
assert user_y == y, "Mismatch user vs storage: var y"
assert user_z == z, "Mismatch user vs storage: var z"
# assert user_A == A, "Mismatch user vs storage: var A"
# assert user_B == B, "Mismatch user vs storage: var B"

### tradeBySource 1 USDC for SHIB - SHIB-side order###
sourceDecimals = 6
targetDecimals = 18
amount = 1

dy, dx, diagnostics = getTradeTargetAmount_bySource(amount * 10**sourceDecimals,storage)
print('TradeBySource')
print(f"Trade {amount} USDC for SHIB")
print('inputAmount', dy, 'outputAmount', dx)
print(dx / 10**targetDecimals, "SHIB")
# print(diagnostics)
if len(diagnostics['len']['error']) > 0:
    raise
print("\n")

### tradeByTarget 10000 SHIB for USDC - SHIB-side order###
sourceDecimals = 6
targetDecimals = 18
amount = 10000
dx, dy, diagnostics = getTradeSourceAmount_byTarget(amount * 10**targetDecimals,storage)
print('TradeByTarget')
print(f"Trade {amount} SHIB for USDC")
print('inputAmount', dx, 'outputAmount', dy)
print(dy / 10**sourceDecimals, "USDC")
# print(diagnostics)
if len(diagnostics['len']['error']) > 0:
    raise
print("\n")
