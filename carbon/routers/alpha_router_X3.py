...
@dataclass
class AlphaRouterX3(BaseRouter):
    """
    ...
    def gen_two_order_selector(amounts, requested_trade_amount, threshold_orders):
        ...
        max_index, max_val = AlphaRouterX3.max_item(indexed_amounts)
        ...
        current_sum = AlphaRouterX3.sum_me(threshold_list)
        ...
        for i in range(threshold_orders,len(amounts)+1):
            ...
            if (not full_fill) & (current_sum==max_fill): # this works because a partial fill requires that the requested_trade_amount is <= max_fill, and max_fill is the n biggest values in the amounts, and thus the current_sum must include the biggest
                exit_summary = 1
                AlphaRouterX3.assertion_checks(full_fill, current_sum, requested_trade_amount, max_fill, threshold_list, threshold_orders)
                sum_threshold_list_indexes = AlphaRouterX3.sum_list_indexes(threshold_list)
            ...
            if current_sum >= requested_trade_amount:
                ...
                min_index, min_val = AlphaRouterX3.min_item_last(threshold_list)
                next_val = AlphaRouterX3.get_i(indexed_amounts, i)[1]

                # the next value to insert must be greater than the current value
                if next_val > min_val:
                    threshold_list.remove(AlphaRouterX3.min_item_last(threshold_list))
                    threshold_list.append(AlphaRouterX3.get_i(indexed_amounts, i))
                else:
                    pass
        exit_summary = 4
        current_sum = AlphaRouterX3.sum_me(threshold_list)
    ...
    def gen_three_order_selector(amounts, requested_trade_amount, threshold_orders):
        ...
        threshold_list, current_sum, requested_trade_amount, sum_threshold_list_indexes, count, exit_summary = AlphaRouterX3.gen_two_order_selector(amounts, requested_trade_amount, threshold_orders)
        if current_sum >= requested_trade_amount:
            full_fill = True
        else:
            full_fill = False
        ...
        if delta > 0:
            ...
            options = AlphaRouterX3.get_replacement_options(delta, indexed_amounts, threshold_list, current_sum, requested_trade_amount)
            # print(options)
        ...
        # return(threshold_list, current_sum, requested_trade_amount, sum_threshold_list_indexes, count, exit_summary)
        return(AlphaRouterX3.list_indexes(threshold_list))     
    ...
    def match_by_target(
        ...
        # else:
        passed_indexes = AlphaRouterX3.gen_three_order_selector(results.ordered_associated_liquidity, abs(x), threshold_orders)
    ...
    def match_by_src(
        ...
        #     top_n_threshold_orders = results[:threshold_orders].indexes.values
        # else:
        passed_indexes = AlphaRouterX3.gen_three_order_selector(results.available_value, abs(x), threshold_orders)
        top_n_threshold_orders = [results.indexes[i] for i in passed_indexes]
